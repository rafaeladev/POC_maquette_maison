...
function Cuisine(props) {
    ...
    const { uBigWavesElevation, uBigWavesFrequencyX, uBigWavesFrequencyY, uBigWavesSpeed } =
    ...
    const { depthColor, surfaceColor, uColorOffset, uColorMultiplier } = useControls(
        'Waves colors',
        {
            depthColor: '#5e98ba',
            surfaceColor: '#c1def5',
        ...
        if (nodes[item.nom] && nodes[item.nom].children) {
            nodes[item.nom].children.forEach((child) => {
                ...
                if (!objetAbimes.some((e) => e.nom === child.name)) {
                    ...
                    ajouterEnfantsSiGroupe({ nom: child.name, shadow: item.shadow }, nodes);
                }
            });
        }
    };
        ...
        return objetInchangeables.map((item, index) => {
            return (
                <primitive
                    ...
                    castShadow={item.shadow === 1 ? true : item.shadow === 2 ? true : false}
                    receiveShadow={item.shadow === 0 ? true : item.shadow === 2 ? true : false}
                />
            );
            /* } */
        });
                ...
                return objetAbimes.map((key, index) => {
                    return (
                        <primitive
                            ...
                            castShadow={key.shadow === 1 ? true : key.shadow === 2 ? true : false}
                            receiveShadow={
